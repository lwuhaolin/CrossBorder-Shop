...
const OrderList: React.FC = () => {
  ...
  const columns: ProColumns<Order>[] = [
    ...
    },
    {
      ...
      width: 120,
      search: false,
      render: (_, record) => `¥${record.totalAmount?.toFixed(2) || "0.00"}`,
    },
    {
      title: "商品金额",
  ...
  const fetchData = async (
    params: OrderListParams & { pageSize?: number; current?: number },
  ) => {
    try {
      const { current, pageSize, ...rest } = params;
      console.log("订单列表请求参数:", { page: current, pageSize, ...rest });
      ...
      console.log("处理后的数据:", { list, total: list.length });
